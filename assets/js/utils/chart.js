(function () {
    ...
    function drawBalloon(balloon, X, Y, consumption, temperature) {
        ...
        context.beginPath();
        var d = 7;
        var W = Number(context.canvas.width); //105;
        var H = Number(context.canvas.height); //55;
    ...
    window.Chart = function (options) {
        ...
        self.rotated = self.alreadyRotated = canvas.getAttribute('data-is-rotated') === 'yes';
        
        var w = context.canvas.width;
        var h = context.canvas.height;
        
        
        context.canvas.width = self.alreadyRotated ? h : w;
        context.canvas.height = self.alreadyRotated ? w : h;
            ...
            for (var i = 0; i < l; i++) {
                if (_mouseX >= stageConsumption[i].x1
                    ...
                    drawBalloon(
						...
						//console.log(context.plot);
                }
				
				
				
		//console.log(data.consDetListCurrGraph[i].electricConsRetDTO.accountId)
        ...
        canvas.eventListeners = canvas.eventListeners || [];
        if (window.addEventListener) {
            for(var i=0;i<canvas.eventListeners.length;i++){
                canvas.removeEventListener('click', canvas.eventListeners[i]);
                canvas.removeEventListener('mousemove', canvas.eventListeners[i]);
            }
            canvas.addEventListener('click', onClickCanvas);
            canvas.addEventListener('mousemove', onClickCanvas);
        } else {
            for(var i=0;i<canvas.eventListeners.length;i++){
                canvas.detachEvent('onclick', canvas.eventListeners[i]);
                canvas.detachEvent('onmousemove', canvas.eventListeners[i]);
            }
        ...
        canvas.eventListeners.push(onClickCanvas);
        ...
        function drawTemperaturePolygon() {
            ...
            for (var i = 0; i < l; i++) {
                ...
                for (var j = 0; j < stageConsumption.length; j++) {
                    if (stageConsumption[j].group == i && stageConsumption[j].temperature) {
                        ...
                        context.strokeStyle = l == 1 ? self.options.colors.temperature[1] : self.options.colors.temperature[i];
                        context.lineWidth = 2;
                        context.fillStyle = l == 1 ? self.options.colors.temperature[1] : self.options.colors.temperature[i];
        ...
        self.rotate = function (r) {
            ...
            if (r == true) {
                ...
                self.rotated = true;
                canvas.setAttribute('data-is-rotated', 'yes');
                ...
                self.rotated = false;
                canvas.setAttribute('data-is-rotated', 'no');
            }
            init();
            context.restore();
        }
